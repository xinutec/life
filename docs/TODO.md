...
- [x] **The queries no test ran** (2026-07-30) — measured, then closed, and the
      reason the `query!` migration was dropped instead of done.
      - **The measurement.** 128 query sites: **110 executed** by a test, **9
        built from compile-time constants** (`concat!`, the per-entity sync SQL,
        trash's per-kind list) and so never macro-eligible anyway, and **9 never
        executed at all**. That last nine was the entire case for `query!`, and
        they were live paths, not dead code: `recipes::list_recipes` (2 — every
        Recipes page load, one a three-table join; the tests built, fetched,
        edited and deleted a recipe but never *listed* them),
        `sync::backfill_shopping` (2 — runs on **every boot**), `todo::restore`,
        `products::set_quantity_label`, `sync`'s todo_link soft-delete, and
        `nextcloud::credentials` (2).
      - **Method**, repeatable and worth keeping: `SET GLOBAL general_log=1,
        log_output='TABLE'` on the dev DB, run the suite, diff the distinct
        `Prepare` statements against the SQL literals in `src/`. It measures
        *execution*, so a compiled-but-unreached query can't hide in it the way
        it can in line coverage.
      - **Closed with five tests**, re-measured the same way: **117 executed**,
        126 of 128 counting the constant-built ones. The two left are
        `nextcloud::credentials`, which need a live Nextcloud — said plainly
        rather than papered over. Each test pins behaviour, not just execution:
        the recipe one asserts an *unlinked* ingredient survives the `LEFT JOIN`
        (an inner join would swallow it) and that another user's recipes don't
        leak; the backfill one asserts a second boot changes no revs, since
        churning them would re-push every row to every device forever; the
        dedupe one asserts *which* duplicate survives, because "lowest id" is
        what makes the cleanup agree across devices.
      - **Why not `query!`:** ~125 eligible sites would each need an entry in a
        committed `.sqlx` cache, regenerated by `cargo sqlx prepare` on every
        query edit and checked in CI, plus `as "col!"` overrides wherever a
        `LEFT JOIN` makes sqlx infer the wrong nullability — a permanent tax on
        every query change to close a gap of nine, still leaving the nine
        constant-built sites unchecked. `verify.sh`'s "nothing else checks them"
        was truer of the comment than of the suite; it now says what's measured.
...
- [x] **Parsed net weight/volume → "how much is left at home"** (2026-08-10) —
      ...
      tiers handed back a product with nothing to fill from. Waitrose supplies
      one too since 2026-08-28: the pack is on the SUMMARY payload's `weights`
      block (`sizeDescription`, "42g" / "100g") rather than beside the name,
      which is why a reader looking near `name` had found nothing. Both import
      paths carry it — the picker's `pickShop` and the product page's refresh,
      which had the same gap. Verified against the live site through
      `scripts/shop-desktop.mjs` on three lineNumbers, not only in a mock.
