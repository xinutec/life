...
- [ ] **The nine queries no test runs** — measured 2026-07-30, and the reason the
      `query!` migration was dropped. `verify.sh` claims *"the queries are
      strings, so nothing else checks them"*; that is truer of the comment than
      of the suite. **128 query sites: 110 are executed by a test, 9 are built
      from compile-time constants (`concat!`, the per-entity sync SQL, trash's
      per-kind list) and so could never use the macros anyway, and 9 are never
      executed at all.** Those nine are the whole gap, and they are live paths,
      not dead code:
      - `recipes::list_recipes` (2) — `GET /api/recipes`, every Recipes page
        load, one of them a three-table join with a `LEFT JOIN`. The tests build,
        fetch, edit and delete a recipe but never **list** them.
      - `sync::backfill_shopping` (2) — runs on **every boot**.
      - `todo::restore` (1) — the trash undelete path for to-dos.
      - `products::set_quantity_label` (1), `sync`'s todo_link soft-delete (1).
      - `nextcloud::credentials` (2) — needs a live Nextcloud; realistically out
        of reach for the suite, and worth saying so rather than pretending.
      **Method** (repeatable): `SET GLOBAL general_log=1, log_output='TABLE'` on
      the dev DB, run the suite, then diff the distinct `Prepare` statements
      against the SQL literals in `src/`. It measures execution rather than line
      coverage, so a compiled-but-unreached query can't hide in it.
      **Why not `query!`:** ~125 eligible sites would each need an entry in a
      committed `.sqlx` cache, regenerated by `cargo sqlx prepare` on every query
      edit and checked in CI, plus `as "col!"` overrides wherever a `LEFT JOIN`
      makes sqlx infer the wrong nullability. That is a permanent tax on every
      query change to close a gap of nine — and it would still leave the nine
      constant-built sites unchecked. Seven tests buy the same protection.
