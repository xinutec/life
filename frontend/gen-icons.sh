...
# Regenerate life's raster icons from the SVG sources in public/.
...
# launcher's choice. Eyeballing the square PNG cannot show that, and the first
# version of icon-maskable.svg failed here by 4.5px — the stroke's outer edge,
# not the path, is what the mask bites into. So assert it.
python3 - <<'PY'
import math, sys
...
for y in range(im.size[1]):
    for x in range(im.size[0]):
        ...
        if r > 200 and g > 200 and b > 200:  # the white house, not the field
...
if worst > SAFE:
    sys.exit(
        ...
        f"Shrink the house in public/icon-maskable.svg."
    )
